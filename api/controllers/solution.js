...
export const register = (req, res) => {
  const { sol_name, sol_full_name, kor_name, n_id, url, github_url, work_field, date, reg_date, imgUrl, usedYN } = req.body;
...
export const likes = (req, res) => {
  const id = req.params.id;
  const updateQuery = "UPDATE special.ITAsset_solutiondata SET likeCnt = likeCnt + 1 WHERE id = ?";
  db.query(updateQuery, [id], (updateErr, updateResult) => {
    ...
    // 업데이트 성공 후 좋아요 수 조회
    const selectQuery = "SELECT likeCnt FROM special.ITAsset_solutiondata WHERE id = ?";
