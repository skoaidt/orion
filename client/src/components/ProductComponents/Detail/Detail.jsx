...
const Detail = ({ selectedRow, currentUser }) => {
  ...
  const [completeStatus, setCompleteStatus] = useState(selectedRow?.complete || null);
  const [comments, setComments] = useState([]);
  const [newComment, setNewComment] = useState('');
  const [showEmojiPicker, setShowEmojiPicker] = useState(false);
  const [commentCount, setCommentCount] = useState(0);

  const isAuthorized = currentUser?.isAdmin || currentUser?.userId === selectedRow.n_id;
  ...
  useEffect(() => {
    const fetchComments = async () => {
      if (selectedRow) {
        try {
          const response = await axios.get(`/api/datatables/pncrgetComments/${selectedRow.id}`);
  ...
  useEffect(() => {
    const fetchCommentCount = async () => {
      try {
        const response = await axios.get(`/api/datatables/countComments/${selectedRow.id}`);
  ...
  const saveCompleteStatus = async () => {
    try {
      await axios.put('/api/datatables/pncrcomplete/', { id: selectedRow.id, complete: completeStatus });
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      ...
      setComments([...comments, {
        id: response.data.id, n_id: currentUser.userId,
        n_name: currentUser.name, team: currentUser.deptName,
        comment: newComment, date: format(new Date(), 'yyyy-MM-dd HH:mm')
      }]);
      setNewComment('');
    } catch (error) {
  ...
  return (
    <div className="detail" >
      ...
      <div className="articleTopBtns">
        ...
        <Button variant="outlined" onClick={handleLinkClick}>목록</Button>
      </div>
      <div className="container" onClick={(e) => e.stopPropagation()}>
        <div className="inner">

          <div className="linkBoard" onClick={handleLinkClick} >PN/CR &gt;</div>
          <div className="titleWrap">

            <div className="title">{selectedRow.title}</div>
            <div className="idCategory">
              <div>ID : {selectedRow.id}</div>
              <div> 분류 [<b>{selectedRow.category}</b>]</div>
            </div>
          </div>

          <div className="writeInfo">
            <div className="left">
              <img src={`${process.env.PUBLIC_URL}/image/icons/noavatar.png`} alt="avatar" />
              <div className="write">
            ...
            <div className="right">
              <div className="comment">
                <span>
                  <img src={`${process.env.PUBLIC_URL}/image/icons/comment.png`} alt="avatar" />
                </span>
          ...
          {isAuthorized && (
            <div className="completeBox">
              ...
              <Button onClick={saveCompleteStatus} variant="outlined" style={{ marginTop: '10px' }}>
                Save
              </Button>
            </div>
          )}
      ...
      {updateOpen && selectedRow && (
        <div className="modalOverlay" style={{ zIndex: 110 }} onClick={() => setUpdateOpen(false)}>
