...
import "./securityInfra.scss";
import CloseIcon from "@mui/icons-material/Close";
import FileUploadIcon from "@mui/icons-material/FileUpload";
import axios from "axios";
import { AuthContext } from "../../../context/authContext";

const SecurityInfra = ({ onClose, ideaId, isViewMode }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data && response.data.infra_URL) {
          setViewData(response.data);
          setUploadedImage(response.data.infra_URL);
        ...
        if (error.response && error.response.status === 404) {
          ...
          setError("저장된 Infra보안진단 데이터를 불러올 수 없습니다.");
        }
      } finally {
        setLoading(false);
        setDataLoaded(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert("Infra보안진단 결과가 성공적으로 등록되었습니다.");
      setViewMode(true);
      onClose(true);
    } catch (error) {
      console.error("Infra보안진단 등록 오류:", error);
    ...
    <div className="securityModalOverlay">
      <div className="securityModalContent">
        ...
        <div className="descriptionBox">
          <p>
            Infra보안진단은 서버OS, DBMS, Web/App서버, WAS, Web/App.
            어플리케이션에 해당합니다.
          </p>
